...
def create_latex_table(model_results, caption=None, label=None):
    ...
    for model_name, task_types in model_results.items():
        ...
        all_scores = [score for task_scores in task_types.values() for score in
                      task_scores.values()]
        if all_scores:
    ...
    header_1 = ["\\textbf{Model} (\\(\\downarrow\\))", "\\textbf{All}", "\\textbf{Type}"] + [
    ...
    task_types_size = [len([x for x in TASK_TYPE_MAPPING.values() if x == task_type]) for task_type
                       in all_task_types]
...
def create_classification_table(model_task_results, caption=None, label=None):
    ...
    task_type = 'Clf'
    all_tasks = sorted(
        {task for tasks in model_task_results.values() for task in tasks[task_type].keys()})
...
def create_all_tasks_table(model_task_results, caption=None, label=None):
    ...
    # Compute best per task for bolding (percent)
    best_per_task = {}
    for t in all_tasks:
...
def create_model_size_scatter(model_results, results_dir=RESULTS_DIR,
                              output_file='model_size_vs_avg.png', annotate=False, log_x=True,
                              annotate_top_n=6, annotate_names=None):
...
def main():
    ...
    output_file = '../sandbox/mteb_results_table.tex'
    ...
    output_file_clf = '../sandbox/mteb_results_table_clf.tex'
    ...
    latex_table_all = create_all_tasks_table(model_results)

    output_file_all = '../sandbox/mteb_results_table_all.tex'
    ...
    create_model_size_scatter(
        model_results, RESULTS_DIR, '../sandbox/model_size_vs_avg.png',
